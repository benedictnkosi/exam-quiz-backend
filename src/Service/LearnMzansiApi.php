...
class LearnMzansiApi extends AbstractController
{
    ...
    public function getQuestionsPerLearnerPerSubject(Request $request): array
    {
        try {
            $fromDate = $request->query->get('from_date');
            ...
            if ($fromDate) {
                $dateTime = \DateTime::createFromFormat('Y-m-d', $fromDate);
                if (!$dateTime || $dateTime->format('Y-m-d') !== $fromDate) {
                    return [
                        'status' => 'NOK',
                ...
                // Default to last 4 weeks if no date provided
                $dateTime = new \DateTime();
                $dateTime->modify('-4 weeks');
            }

            $qb = $this->em->createQueryBuilder();
            $qb->select('l.name as capturer_name, s.name as subject_name, q.status, COUNT(q.id) as question_count')
                ->from('App\Entity\Question', 'q')
                ->join('q.capturer', 'l')
                ->join('q.subject', 's')
                ->where('q.created >= :fromDate')
                ->groupBy('l.id, l.name, s.id, s.name, q.status')
                ->orderBy('l.name', 'ASC')
                ->addOrderBy('s.name', 'ASC');

            $results = $qb->getQuery()
                ->setParameter('fromDate', $dateTime)
                ->getResult();

            // Format the results
            $formattedResults = [];
            foreach ($results as $result) {
                $capturerName = $result['capturer_name'];
                $subjectName = $result['subject_name'];
                $status = $result['status'];

                if (!isset($formattedResults[$capturerName])) {
                    $formattedResults[$capturerName] = [
                ...
                $subjectFound = false;
                foreach ($formattedResults[$capturerName]['subjects'] as &$subject) {
                    if ($subject['subject'] === $subjectName) {
                        $subject['counts'][$status] = (int) $result['question_count'];
                        $subject['total'] += (int) $result['question_count'];
                        $subjectFound = true;
                        break;
                    }
                }
                ...
                if (!$subjectFound) {
                    $formattedResults[$capturerName]['subjects'][] = [
                        'subject' => $subjectName,
                        'counts' => [
                            'new' => 0,
                            'approved' => 0,
                            'rejected' => 0,
                            $status => (int) $result['question_count']
                        ],
                        'total' => (int) $result['question_count']
                    ];
                }

                $formattedResults[$capturerName]['total_questions'] += (int) $result['question_count'];
            }

            // Convert to indexed array and sort by total questions
            $finalResults = array_values($formattedResults);
            ...
            return [
                'status' => 'NOK',
                'message' => 'Error getting questions per learner per subject: ' . $e->getMessage()
            ];
        }
    }
}
