...
#[AsCommand(
    name: 'app:ewn-madlanga-video',
    description: 'Create video from EWN Madlanga Commission of Inquiry stream'
)]
class EwnMadlangaVideoCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln('<info>Searching for Madlanga Commission of Inquiry videos...</info>');

        // Find Madlanga Commission video
        [$videoId, $title] = $this->findMadlangaCommissionVideo($days);

        if (!$videoId) {
            $output->writeln('<error>No Madlanga Commission of Inquiry video found</error>');
        ...
        $transcriptFile = $transcriptDir . '/madlanga_' . $videoId . '_' . date('Y-m-d_H-i-s') . '.txt';
        ...
        // Generate 60-second script with Ammy as anchor using full transcript
        $output->writeln('<info>Generating 60-second script...</info>');
        $script = $this->openAIService->generateVideoScriptFromTranscript($transcriptContent, 10, 60, 'Ammy');
        ...
        $this->logger->info('Madlanga Commission script', [
            'videoId' => $videoId,
            'title' => $title,
            'script' => $script,
            'fileId' => $fileId
    ...
    private function findMadlangaCommissionVideo(int $daysBack): array
    {
        try {
        ...
        // Search for Madlanga Commission videos (YouTube already sorts by newest first)
        $needle = 'madlanga commission';
        $queue = [$data];
        $totalVideosFound = 0;
        $matchingVideosFound = 0;
        $dateFilteredVideos = 0;
        
        $this->logger->info('Starting Madlanga Commission video search', [
            'searchTerm' => $needle,
            'daysBack' => $daysBack
        ]);
        ...
        $this->logger->info('Madlanga Commission search completed', [
