...
class LearnMzansiApiController extends AbstractController
{
    ...
    #[Route('/learn/questions/parent', name: 'get_questions_by_parent', methods: ['GET'])]
    public function getQuestionsByParentId(Request $request): JsonResponse
    {
        $this->logger->info("Starting Method: " . __METHOD__);
        $response = $this->api->getQuestionsByParentId($request);
    ...
    public function getRandomQuestionBySubjectName(Request $request): JsonResponse
    {
        ...
        $response = $this->api->getRandomQuestionBySubjectName($subjectName, $paperName, $uid, $questionId, $platform, $mode);
        $context = SerializationContext::create()->enableMaxDepthChecks();
        $jsonContent = $this->serializer->serialize($response, 'json', $context);
        return new JsonResponse($jsonContent, 200, array('Access-Control-Allow-Origin' => '*'), true);
    }
