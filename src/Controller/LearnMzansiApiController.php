...
class LearnMzansiApiController extends AbstractController
{
    ...
    public function getRandomQuestionBySubjectName(Request $request): JsonResponse
    {
        ...
        if ($subscriptionCheck) {
            $question = $this->api->getRandomQuestionBySubjectName($subjectName, $paperName, $uid, $questionId, $platform, $topic, $subscriptionCheck, $version);
        } else {
            $question = $this->api->getRandomQuestionBySubjectName($subjectName, $paperName, $uid, $questionId, $platform, $topic, $version);
        }
    ...
    #[Route('/learn/chat/upload-lecture-image', name: 'upload_lecture_image', methods: ['POST'])]
    public function uploadLectureImage(Request $request): JsonResponse
    {
        $this->logger->info("Starting Method: " . __METHOD__);
        $response = $this->api->uploadLectureImage($request);
        $context = SerializationContext::create()->enableMaxDepthChecks();
        $jsonContent = $this->serializer->serialize($response, 'json', $context);
        return new JsonResponse($jsonContent, 200, array('Access-Control-Allow-Origin' => '*'), true);
    }
    ...
    #[Route('/learn/learner/get-lecture-image', name: 'get_lecture_image', methods: ['GET'])]
    public function getLectureImage(Request $request): Response
    {
        $this->logger->info("Starting Method: " . __METHOD__);
        ...
        $uploadDir = __DIR__ . '/../../public/assets/images/lectures/';
        $imagePath = $uploadDir . $imageName;
    ...
    #[Route(path: '/learn/question/recording/{subjectName}/{grade}', name: 'remove_recorded_questions', methods: ['DELETE'])]
    ...
    #[Route('/learn/messages', name: 'get_messages', methods: ['GET'])]
